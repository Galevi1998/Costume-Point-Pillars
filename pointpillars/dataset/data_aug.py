...
def dbsample(CLASSES, data_root, data_dict, db_sampler, sample_groups):
    ...
    for name, v in sample_groups.items():
        ...
        # print(f"\n🔍 Number of sampled objects: {len(sampled_cls_list)}")
        # for i, item in enumerate(sampled_cls_list):
        #     print(f"   ▪ Sample {i} name: {item['name']} | box3d_lidar: {item['box3d_lidar']} | shape: {np.array(item['box3d_lidar']).shape}")
        sampled_cls_bboxes = np.array([item['box3d_lidar'] for item in sampled_cls_list], dtype=np.float32)
        # print(f"📦 sampled_cls_bboxes shape: {sampled_cls_bboxes.shape} | dtype: {sampled_cls_bboxes.dtype}")
        # print(f"📦 sampled_cls_bboxes: {sampled_cls_bboxes}")
    ...
    if len(sampled_bboxes) > 0:
        ...
        # אם לא נוספו אובייקטים מה-db, פשוט המשך עם מה שיש
        gt_bboxes_3d = avoid_coll_boxes.astype(np.float32)
