...
#     '''
#     bboxes: shape=(N, 7) — [x, y, z, h, w, l, ry] in camera coords
#     tr_velo_to_cam: (4, 4)
#     r0_rect: (4, 4)
#     return: (N, 7) in lidar coords — [x, y, z, l, w, h, ry]
#     '''
#     bboxes = bboxes.copy()

#     # print("\n🟡 [DEBUG] Original bboxes (camera coords, bottom-center):")
#     # for i, b in enumerate(bboxes):
#     #     print(f"  ▪ Box {i}: center(xyz)={b[:3]}, hwl={b[3:6]}, ry={b[6]:.2f}")

#     # KITTI → תיבת bottom-center → center
#     bboxes[:, 1] -= bboxes[:, 3] / 2  # y -= h/2

#     # print("\n🔵 [DEBUG] Adjusted bboxes (centered at box center, for LiDAR):")
#     # for i, b in enumerate(bboxes):
#     #     print(f"  ▪ Box {i}: adjusted y = {b[1]:.2f} (after subtracting h/2 = {b[3]/2:.2f})")

#     # Extract and reorder dims
#     x_size = bboxes[:, 5:6]  # length
#     y_size = bboxes[:, 4:5]  # width
#     z_size = bboxes[:, 3:4]  # height
...
#     # print("\n🔺 [DEBUG] Transformation matrix (camera → LiDAR):\n", rt_mat)

#     xyz = extended_xyz @ rt_mat.T

#     # print("\n🟢 [DEBUG] Converted centers in LiDAR coords:")
#     # for i, xyz_i in enumerate(xyz[:, :3]):
#     #     print(f"  ▪ Box {i}: lidar xyz = {xyz_i}")

#     bboxes_lidar = np.concatenate([xyz[:, :3], xyz_size, bboxes[:, 6:]], axis=1)

#     # print("\n✅ [DEBUG] Final Lidar-format bounding boxes [x, y, z, l, w, h, yaw]:")
#     # for i, b in enumerate(bboxes_lidar):
#     #     print(f"  ▪ Box {i}: {b}")
...
def bbox_camera2lidar(bboxes):
    """
    Use this when your 3D labels are already in the same coordinate system as the LiDAR.
    
    Input:
        bboxes: np.ndarray of shape (N, 7) with KITTI-style format:
                [x, y, z, h, w, l, yaw] in camera coords (bottom-center)

    Output:
        bboxes_lidar: np.ndarray of shape (N, 7) in LiDAR coords (center-based)
                      [x, y, z, l, w, h, yaw]
    """
    bboxes = bboxes.copy()
    
    # 1. Convert from bottom-center to box-center in height (z)
    bboxes[:, 1] -= bboxes[:, 3] / 2  # y -= height / 2  (KITTI uses bottom-center as y)

    # 2. Reorder dimensions from hwl → lwh (LiDAR format expects l, w, h)
    l = bboxes[:, 5:6]  # length
    w = bboxes[:, 4:5]  # width
    h = bboxes[:, 3:4]  # height
    xyz_size = np.concatenate([h, w, l], axis=1)

    # 3. Assemble new bboxes: [x, y, z, l, w, h, yaw]
    bboxes_lidar = np.concatenate([bboxes[:, :3], xyz_size, bboxes[:, 6:]], axis=1)

    return bboxes_lidar.astype(np.float32)
...
def points_in_bboxes_v2(points, r0_rect, tr_velo_to_cam, dimensions, location, rotation_y, name):
    ...
    bboxes_lidar = bbox_camera2lidar(bboxes_camera)
    # bboxes_lidar = bbox_camera2lidar(bboxes_camera, tr_velo_to_cam, r0_rect)
    # print("✅ bbox_camera2lidar done | all boxes for fram 00001:", bboxes_lidar)
...
def get_points_num_in_bbox(points, r0_rect, tr_velo_to_cam, dimensions, location, rotation_y, name):
    ...
    indices, n_total_bbox, n_valid_bbox, bboxes_lidar, name = \
        points_in_bboxes_v2(
            ...
            name=name)
    # visualize(points, bboxes_lidar)
    # visualize_pointcloud_with_boxes(points, bboxes_lidar)
    for i, box in enumerate(bboxes_lidar):
        num_points = np.sum(indices[:, i])
        print(f"🔍 Box {i} @ {np.round(box[:3], 2)} | Size: {np.round(box[3:6], 2)} | Points inside: {num_points}")
...
def remove_outside_points(points, r0_rect, tr_velo_to_cam, P2, image_shape):
    ...
    # print(f"🔍 Box {np.round(b[:3], 2)} | Points inside: {num}")
...
#==================== Gal's function for get pointcloud filtered without calibration ======================================

def filter_points_by_range(points, x_min, x_max, y_min, y_max, z_min, z_max):
    """
    Filters a point cloud array to keep only points inside the given XYZ bounds.
    
    Args:
        points: (N, 4) numpy array of point cloud [x, y, z, reflectance]
        x_min, x_max, y_min, y_max, z_min, z_max: float bounds
    
    Returns:
        filtered_points: (M, 4) numpy array
    """
    mask = (
