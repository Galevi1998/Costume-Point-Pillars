...
def read_label(file_path):
    ...
    # סינון שורות לא תקינות או ריקות
    lines = []
    for i, line in enumerate(lines_raw):
        split_line = line.strip().split()
        if len(split_line) < 15:
            print(f"⚠️ Skipping invalid line in {file_path}, line {i}: {split_line}")
            continue
        lines.append(split_line)

    # אם אין שורות תקינות – החזר מבנה ריק
    if len(lines) == 0:
        return {
            'name': np.array([]),
            'truncated': np.array([]),
    ...
    # יצירת annotation תקני
    # print(f"before adjusting the dimensions : {np.array([line[8:11] for line in lines], dtype=np.float32)}")
    annotation = {
        'name': np.array([line[0] for line in lines]),
        'truncated': np.array([line[1] for line in lines], dtype=np.float32),
        'occluded': np.array([line[2] for line in lines], dtype=np.int32),
        'alpha': np.array([line[3] for line in lines], dtype=np.float32),
        'bbox': np.array([line[4:8] for line in lines], dtype=np.float32),
        'dimensions': np.array([line[8:11] for line in lines], dtype=np.float32)[:, [2, 1, 0]], 
        'location': np.array([line[11:14] for line in lines], dtype=np.float32),
        'rotation_y': np.array([line[14] for line in lines], dtype=np.float32),
    }
    # print(f"after adjusting the dimensions : {np.array([line[8:11] for line in lines], dtype=np.float32)[:, [2, 1, 0]]}")
