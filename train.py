...
def main(args):
    ...
    if args.transfer_from is not None:
        print(f"\n🔁 Loading pretrained weights from {args.transfer_from}")
        checkpoint = torch.load(args.transfer_from)
        pointpillars.load_state_dict(checkpoint, strict=True)

        print("\n🔒 Freezing feature extractor layers (pillar_encoder, backbone, neck)...")
        for param in pointpillars.pillar_encoder.parameters():
            param.requires_grad = False
        for param in pointpillars.backbone.parameters():
            param.requires_grad = False
        for param in pointpillars.neck.parameters():
            param.requires_grad = False
    ...
    for epoch in range(args.max_epoch):
        ...
        if (epoch + 1) % args.ckpt_freq_epoch == 0:
            torch.save(pointpillars.state_dict(), os.path.join(saved_ckpt_path, f'epoch_{epoch+1}_overffiting.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--transfer_from', default=None, help='Path to pretrained weights for transfer learning')
